...
<html lang="en">
    ...
    <body>
        ...
        <div class="add">
            ...
            <form method="post" action="Marks.jsp" id="f">
                ...
                <select name="Subject" id="sub">
                    ...
                    sc = request.getParameter("cls");
                                ...
                                session.setAttribute("select",sc);
                                        ResultSet r = s.executeQuery("SELECT * FROM subinfo where cid = '"+sc+"'");
                                        while(r.next()){
...
+ "values('"+rollno+"','"+subject+"','"+"FINAL"+"','"+of+"','"+tf+"')");
    ...
    <div class="view">
        ...
        <br>
        <form method="post" action="Marks.jsp" id="cc">
           ...
           try{
            ...
            s.close();
                ...
                <button type="submit" name="bx" class="bx">Done</button>
        </form>
                <%
                if(request.getParameter("bx")!=null){
                %>
       <script>document.getElementById("cc").hidden = true;
            </script>
                <form method="post" action="Marks.jsp">
            <input type="submit" name="b4" class="v" value="View">
            <br>
            <label class="cls">Class : <%=request.getParameter("class")%> {<%=request.getParameter("class")%>}
            </label>
            <br>
    ...
    try{
                ...
                Connection c = DriverManager.getConnection("jdbc:mysql://localhost:3306/schooldb?useSSL=false","root","");
                        ...
                        Statement s = c.createStatement();
                        ResultSet r = 
                s.executeQuery("select * from stuinfo where cid = '"+request.getParameter("class")+"'");
                while(r.next()){
                String name = r.getString(2);
                String roll = r.getString(1);
                std.put(roll, name);
...
if(request.getParameter("b4")!=null){%><script>document.getElementById("cc").hidden = true;
            </script><%
if(request.getParameter("std")==""){
            out.println("<script>");
            out.println("alert('Please select a student!');");
            out.println("location='Marks.jsp';");
            out.println("</script>");
            }else{
